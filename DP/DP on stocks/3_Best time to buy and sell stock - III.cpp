...
class Solution {
   private:
    int func(int id, int buy, vector<int> arr, int n, vector<vector<vector<int>>>& dp, int buyCount) {
        ...
        if (buy) {
            ...
            int notBoughtStockOnCurrentDay = func(id + 1, 1, arr, n, dp, buyCount);

            profit = max(boughtStockOnCurrentDay, notBoughtStockOnCurrentDay);
        } else {
            int soldStockOnCurrentDay = arr[id] + func(id + 1, 1, arr, n, dp, buyCount - 1);
            int notSoldStockOnCurrentDay = func(id + 1, 0, arr, n, dp, buyCount);
